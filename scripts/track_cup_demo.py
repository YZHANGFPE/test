...
class track():
    ...
    def track(self, initial_pose, hdr, arm, id, timeout = 120):
        ...
        while self.finish != True:
            ...
            z = resp.robotz - 0.3
            time += 1
            if self.found == 1 and abs(x)< 1 and abs(y)< 1 and abs(z)<1:
                #if self.distance > 0.10:
                vx = (x - current_pose.pose.position.x)/100
                vy = (y - current_pose.pose.position.y)/100
                vz = (z - current_pose.pose.position.z)/100
                solution_found, predict_pose = ik_move_one_step(initial_pose, predict_pose, hdr, arm, self.kin, target_dx = min(0.01,vx), target_dy = min(0.01,vy), target_dz = min(0.01,vz))
